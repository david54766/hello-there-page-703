...
function ScriptsPage() {
  ...
  const [draft, setDraft] = useState<{ contractor_type: string; kind: "hello" | "system" | "first_message"; label: string; body: string }>({ contractor_type: "", kind: "system", label: "", body: "" });
  const [saving, setSaving] = useState(false);

  const reload = async () => {
    const { templates } = await fetchTemplates({ data: {} });
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      await upsert({ data: { contractorType: draft.contractor_type || undefined, kind: draft.kind, label: draft.label, body: draft.body } });
      setDraft({ contractor_type: "", kind: "system", label: "", body: "" });
      await reload();
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6 sm:py-10 pb-24 md:pb-10">
      ...
      <p className="mb-6 text-sm text-muted-foreground">Reusable greetings, first messages and full system prompts. Tag with a contractor type to filter when picking on the Vapi page.</p>
      ...
      {loading ? (
        ...
        <Card className="p-4 text-sm text-muted-foreground">No templates yet.</Card>
      ) : (
